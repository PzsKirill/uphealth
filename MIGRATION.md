...
- [ ] Supliful-биллинг: карта привязана, **платный план активен**
      (фулфилмент не работает на бесплатном; себестоимость+доставка
      списываются с карты при каждом заказе)
...
- [ ] **Авто-фулфилмент**: Settings → Checkout → Order processing →
      «Automatically fulfill the order's line items» — иначе заказы висят,
      пока не нажат Request fulfillment, и Supliful их не видит
- [ ] Зоны/тарифы доставки = регионам Supliful. Рекомендация: запуск
      **US-only** (их родной рынок; международка = FedEx из США, DDU —
      пошлины платит клиент; часть товаров US-only → отдельный
      shipping profile). Клиентская цена доставки на чекауте — НАША
      настройка, Supliful выставляет доставку только нам
