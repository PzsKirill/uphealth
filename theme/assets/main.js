...
(function () {
  ...
  /* ---------- Product page: variant + bottle packs + subscription + add to cart ---------- */
  (function initProductBuy() {
    const buy = $('.showcase__buy');
    if (!buy) return;                       // product page only

    const SUB_RATE  = 0.10;                 // extra discount layered on the chosen pack
    ...
    const packEls    = $$('[data-packs] .pack');
    const packInputs = $$('[data-packs] input[name="pack"]');

    // The selected pack carries qty + per-bottle price via data-attributes.
    const current = () => packInputs.find((r) => r.checked) || packInputs[0];
    const packData = () => {
      const input = current();
      return {
        qty:  parseInt(input?.dataset.packQty, 10) || 1,
        each: parseFloat(input?.dataset.packEach) || 0,
      };
    };

    // Recompute the buy block from current state (selected pack + subscription)
    const refresh = () => {
      const { qty, each } = packData();
      const packTotal = each * qty;
      const perUnit   = sub ? each * (1 - SUB_RATE) : each;
      if (priceNow) priceNow.textContent = money(perUnit * qty);          // what you'll pay
      if (priceOld) priceOld.textContent = money(unitOld * qty);
      if (subSave)  subSave.textContent  = '−' + money(packTotal * SUB_RATE);
      if (subEl)    subEl.classList.toggle('is-active', sub);
      if (addBtn)   addBtn.textContent   = sub ? 'Start subscription' : 'Add to cart';
      packEls.forEach((p) => p.classList.toggle('is-active', p.querySelector('input').checked));
    };
    ...
    // Bottle pack (1 bottle / 3 bottles) — sets the quantity and base price
    packInputs.forEach((r) => r.addEventListener('change', refresh));

    // Subscribe & save another 10%
    ...
    addBtn && addBtn.addEventListener('click', (e) => {
      e.preventDefault();
      const { qty, each } = packData();
      const perUnit = Math.round((sub ? each * (1 - SUB_RATE) : each) * 100) / 100;
      window.UpCart && window.UpCart.add({
        id: title + '|' + variant + '|' + qty + '|' + perUnit + (sub ? '|sub' : ''),
